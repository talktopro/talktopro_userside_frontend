...
const usePayment = () => {
   ...
   const getRazorPayInstance = (options: IRazorpayOptions) => {
      return new (window as Window & typeof globalThis & { Razorpay: any }).Razorpay(options);
   };

   //! ========================================== Creates a Razorpay order and retrieves the booking ID ===========================================
   const getRazorpayOrder = async (amount: number, mentorId: string, date: Date, slot: string) => {
      try {
         const { data } = await apiClient.post<IRazorpayOrderResponse>(`/bookings`, {
            mentor_id: mentorId,
            slot: {
   ...
   const handleTriggerPayment = async (date: Date, slot: string, mentor: IMentorDetailsWithSlots, setShowPaymentSuccess: React.Dispatch<React.SetStateAction<boolean>>) => {
      try {
         ...
         if (!amount || !currency || !order_id || !_id) {
